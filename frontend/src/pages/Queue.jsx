...
function Queue() {
  ...
  const handleAction = (queueItem) => {
    ...
    // [FIX] Pass 'appointment' object so PatientForm knows this is a NEW session
    // This prevents the form from auto-filling old Vitals from the patient record.
    const appointmentPayload = {
      ...queueItem,
      dentist_id: queueItem.dentist_id,
      // Map 'notes' from queue to 'procedure' for the timeline autofill
      procedure: queueItem.notes,
    ...
    navigate(`/app/patient/${queueItem.patient_id}/`, {
      state: {
        ...
        status: queueItem.status,
        patientData: fullPatientData,
        appointment: appointmentPayload // <--- This is the key fix
      }
    });
  };
